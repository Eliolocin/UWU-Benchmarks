...
class GPT:
    ...
    def generate_roleplay(character, config):
        ...
        cot_response = client.chat.completions.create(
                    messages=[
                        {"role": "system", "content": "You are a helpful and expert roleplayer."},
                        {"role": "user", "content": prompt},
                    ],
                    model=f"{config.test_llm}",
                )
        ##############################

        cot_generation = cot_response.choices[0].message.content
        ...
        while conversation_turns < config.rp_turns*2:
            ...
            while retries > 0:
                ...
                response = client.chat.completions.create(
                    messages=[
                        {"role": "system", "content": "You are a helpful and expert roleplayer."+help_prompt},
                ...
                # print("Unclean text: "+current_speaker+": "+response.choices[0].message.content)
                generated_text = GPT.trim_response(current_speaker+": "+response.choices[0].message.content)
                # print(generated_text)  # Debug: Print the generated text


                # Ensure proper formatting
                if GPT.is_valid_response(current_speaker, generated_text):
