...
class Claude:
    ...
    def generate_roleplay(character, config):
        ##############################
        client = Anthropic(api_key=config.claude_key)
        ##############################
        ...
        cot_response = client.messages.create(
                    max_tokens=1024,
                    messages=[
                        {"role": "user", "content": "You are a helpful and expert roleplayer."},
        ...
        cot_generation = cot_response.content[0].text
        ...
        while conversation_turns < config.rp_turns*2:
            ...
            while retries > 0:
                ...
                response = client.messages.create(
                    max_tokens=1024,
                    messages=[
                        {"role": "user", "content": "You are a helpful and expert roleplayer."+help_prompt},
                ...
                generated_text = Claude.trim_response(current_speaker+": "+response.content[0].text)
                # print(generated_text)  # Debug: Print the generated text


                # Ensure proper formatting
                if Claude.is_valid_response(current_speaker, generated_text):
